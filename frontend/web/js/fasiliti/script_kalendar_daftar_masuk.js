...
{
  var url = "/fasiliti/daftar-masuk/get-checkin-tahunan";
}
else
{
  var url = "/yktlj/frontend/web/fasiliti/daftar-masuk/get-checkin-tahunan";
}
...
$.ajax({
    //type: "GET",
    //url: '../../../fasiliti/daftar-masuk/get-checkin-tahunan', 
    //url: namaSistem + '/frontend/web/fasiliti/daftar-masuk/get-checkin-tahunan',
    url: url,   
    ...
    success: function (doc) {
        $(doc).each(function() {
            ...
            {
                id: $(this).attr('id'),
                name: $(this).attr('pesakit').psk_mod_nama,
                startDate: new Date($(this).attr('cio_mod_tarikh_check_in')),
                endDate: new Date($(this).attr('cio_mod_tarikh_check_out')),
                status: $(this).attr('cio_mod_status_check_out'),
                catatan: $(this).attr('cio_mod_catatan'),
                aras: $(this).attr('fasiliti').fsl_mod_aras,
                bilik: $(this).attr('fasiliti').fsl_mod_bilik,
                katil: $(this).attr('fasiliti').fsl_mod_no_fasiliti,
                color: color,
            }
            ...
            if(e.events.length > 0) {
                ...
                for(var i in e.events) {
                    ...
                    content += '<div class="event-tooltip-content">'
                                    ...
                                    + '<div class="">' + e.events[i].catatan + '</div>'
                                    + '<div class=""><b>Aras : </b>' + e.events[i].aras + '</div>'
                                    + '<div class=""><b>Bilik : </b>' + e.events[i].bilik + '</div>'
                                    + '<div class=""><b>Katil : </b>' + e.events[i].katil + '</div>'
                                + '</div>';
